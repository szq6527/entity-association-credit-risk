#!/usr/bin/env python3
"""
Improved DGL HTGNN training for rating downgrade prediction.

Improvements vs train_htgnn_dgl.py:
- richer node features (graph degree stats, log transforms, lag/delta features)
- relation-level attention in spatial encoder
- temporal attention on top of GRU
- selectable loss (focal BCE or weighted BCE)
- configurable relation set and optional tabular residual branch
- AdamW + LR scheduler + early stopping
...
def build_features(node_frames: Dict[int, pd.DataFrame], train_year_end: int) -> Tuple[Dict[int, torch.Tensor], int]:
    ...
    skew_cols = [c for c in ["Regcap", "total_assets", "total_liabilities", "total_equity", "revenue_total", "revenue_main", "cashflow_operating", "guar_amt_sum", "guar_amt_mean"] if c in num_cols]
    for c in skew_cols:
        full[f"log1p_{c}"] = np.log1p(np.clip(full[c].fillna(0).to_numpy(dtype=np.float64), a_min=0, a_max=None))

    # Lag and change features.
    lag_base = [c for c in ["total_assets", "total_liabilities", "total_equity", "revenue_total", "cashflow_operating", "asset_liability_ratio", "guar_event_cnt", "guar_amt_sum", "rating_event_cnt"] if c in full.columns]
