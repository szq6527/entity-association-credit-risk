...
export PYTHONPATH="$ROOT_DIR/third_party/python${PYTHONPATH:+:$PYTHONPATH}"
...
if [[ "${RUN_DGL_IMPROVED:-0}" == "1" ]]; then
  run_step "Train improved DGL HTGNN (optional)" \
    python3 scripts/train_htgnn_dgl_improved.py --device cpu --exp-name v10_all_notab_bce_w6_h128 --relations guarantee,equity_assoc,co_controller --no-tabular-residual --window 6 --hidden-dim 128 --dropout 0.25 --lr 5e-4 --weight-decay 8e-4 --loss bce --epochs 220 --patience 45 --seed 42
fi
